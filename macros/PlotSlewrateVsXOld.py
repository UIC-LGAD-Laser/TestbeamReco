...
outdirTmp = myStyle.GetPlotsDir(outdir, "slewRate_vs_X_fits/")
...
for channel in range(0, len(list_slewRate_vs_x)):
    ...
    for i in range(1, list_slewRate_vs_x[channel].GetXaxis().GetNbins()+1):
        ...
        nXBins = th1_Nbins
        minEvtsCut = totalEvents/nXBins #800 # Minimum value chosen to account for the slewRate values at the strip (for laser source)
...
outputfile = TFile("%sPlotSlewRateVsX.root"%(outdir),"RECREATE")
...
#Make final plots
plotfile = TFile("%sPlotSlewRateVsX.root"%(outdir),"READ")
