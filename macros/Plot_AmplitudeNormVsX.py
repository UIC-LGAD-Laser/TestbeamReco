...
parser.add_option('-y','--ylength', dest='ylength', default = 1.5, help="Max Amp value in final plot")
...
outdirTmp = myStyle.GetPlotsDir(outdir, "AmpNorm_vs_X_fits/")
...
outputfile = TFile("%sPlotAmplitudeNormalizedVsX.root"%(outdir),"RECREATE")
# for channel in range(0, len(list_amplitude_vs_x)):
#     list_amplitude_vs_x[channel].Write()

# Normalize the histograms
for iter in range(len(list_amplitude_vs_x)):
    max_amplitude = list_amplitude_vs_x[iter].GetMaximum()
    print(max_amplitude)
    for i in range(1, list_amplitude_vs_x[iter].GetXaxis().GetNbins()+1):
        temp = list_amplitude_vs_x[iter].GetBinContent(i)/max_amplitude
        list_amplitude_vs_x[iter].SetBinContent(i, temp)
...
#Make final plots
plotfile = TFile("%sPlotAmplitudeNormalizedVsX.root"%(outdir),"READ")
...
canvas.SaveAs(outdir+"TotalAmplitudeNormalized_vs_x_"+sensor+".gif")