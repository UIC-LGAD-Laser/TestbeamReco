...
regions = ['midgap', 'nearStrip']
max_save = 10
FileSave = ['Midgap', 'Near-strip']

for region_iter in range(len(regions)):
    for waveform_iter in range(max_save):    
        filename = outdir+"/waveforms/waveform_"+regions[region_iter]+"_"+str(waveform_iter)+".csv"
    ...
    plt.grid()
    plt.savefig(outdir+"/waveforms/Plot"+FileSave[region_iter]+".png")
    plt.close()
