...
error1=list_baselineRMS_vs_x[2].GetBinError(list_baselineRMS_vs_x[2].FindBin(-0.25))
temp2=list_baselineRMS_vs_x[1].GetBinContent(list_baselineRMS_vs_x[1].FindBin(-0.25))
error2=list_baselineRMS_vs_x[1].GetBinError(list_baselineRMS_vs_x[1].FindBin(-0.25))
...
with open(filename, 'w') as trfile:
    for line in lines:
        if line.startswith(f'{dataset}:'):
            line = line.strip() + f': {round(midgapnoise, 2)}, {round(midgaperror, 2)}\n'
        trfile.write(line)
