...
parser.add_option('-y','--ylength', dest='ylength', default = 300, help="Max slewRate value in final plot")
...
outdirTmp2 = myStyle.GetPlotsDir(outdirTmp1, "SlewRate_vs_X_fits/")

#Get 3D histograms
channel_good_index = []
th3_slewRate_vs_xy_ch = []
for i in range(3):
    hname = "slewRate_vs_xy_channel0"+str(i)
    if inputfile.Get(hname):
        channel_good_index.append(i)
        th3_slewRate_vs_xy_ch.append(inputfile.Get(hname))


shift = 0 #inputfile.Get("stripBoxInfo03").GetMean(1)

#Build 2D amp vs x histograms
list_th2_slewRate_vs_x = []
for i,ch in enumerate(channel_good_index):
    list_th2_slewRate_vs_x.append(th3_slewRate_vs_xy_ch[i].Project3D("zx"))

#Build slewRate histograms
th1 = th3_slewRate_vs_xy_ch[0].ProjectionX().Clone("th1")
...
for i,ch in enumerate(channel_good_index):
    list_slewRate_vs_x.append(TH1F("slewRate_vs_x_channel0%i"%(ch),"", th1_Nbins, th1_Xmin, th1_Xmax))
...
n_channels = 0
#loop over X,Y bins
for channel in range(0, len(list_slewRate_vs_x)):
    totalEvents = list_th2_slewRate_vs_x[channel].GetEntries()
    for i in range(1, list_slewRate_vs_x[channel].GetXaxis().GetNbins()+1):
        tmpHist = list_th2_slewRate_vs_x[channel].ProjectionY("py",i,i)
        ...
        nXBins = th1_Nbins
        minEvtsCut = totalEvents/nXBins # Minimum value chosen to account for the slewRate values at the strip (for laser source)
        ...
        list_slewRate_vs_x[channel].SetBinContent(i,value)
                    
# Save slewRate histograms
outputfile = TFile("%sPlotSlewRateVsX.root"%(outdirTmp1),"RECREATE")

for hist in list_slewRate_vs_x:
    hist.Write()
...
#Make final plots
plotfile = TFile("%sPlotSlewRateVsX.root"%(outdirTmp1),"READ")
plotList_slewRate_vs_x  = []
for i,ch in enumerate(channel_good_index):
    plot_slewRate = plotfile.Get("slewRate_vs_x_channel0%i"%ch)
    plot_slewRate.SetLineWidth(2)
    plot_slewRate.SetLineColor(colors[i])
    plotList_slewRate_vs_x.append(plot_slewRate)


tempslewRate_vs_x = TH1F("htemp","",1,-xlength,xlength)
tempslewRate_vs_x.Draw("hist")
tempslewRate_vs_x.SetStats(0)
tempslewRate_vs_x.SetTitle("")
tempslewRate_vs_x.GetXaxis().SetTitle("Track x position [mm]")
tempslewRate_vs_x.GetYaxis().SetTitle("Slewrate [mV/ns]")
tempslewRate_vs_x.SetLineWidth(2)

tempslewRate_vs_x.SetMaximum(ylength)

# boxes = getStripBox(inputfile,0,ylength-10.0,False, 18, True, shift)
# for box in boxes:
#    box.Draw()
tempslewRate_vs_x.Draw("AXIS same")
tempslewRate_vs_x.Draw("hist same")
...
for i,ch in enumerate(channel_good_index):
    plotList_slewRate_vs_x[i].Draw("hist same")
    legend.AddEntry(plotList_slewRate_vs_x[i], "Strip %i"%(ch+1))
legend.Draw()

# myStyle.BeamInfo()
myStyle.SensorInfoSmart(dataset)

canvas.SaveAs(outdir+"slewRate_vs_x_"+sensor+".gif")