...
parser.add_option('-y','--ylength', dest='ylength', default = 150.0, help="Max TimeResolution value in final plot")
...
outdir = myStyle.GetPlotsDir(outdir, "TimeResForSimulation/")
...
all_histoInfos = [
    ...
    HistoInfo("t1_res_vs_xy", inputfile, "t1_res"),
    HistoInfo("t2_res_vs_xy", inputfile, "t2_res"),
...
if debugMode:
    outdir_q = myStyle.GetPlotsDir(outdir, "TimeResFits/")
...
for i in range(1, nXBins+1):
    ...
    for info in all_histoInfos:
        ...
        if("W9" in dataset):
            minEvtsCut = 0.5*totalEvents/nXBins # changed from 0.3X to 1.5X to account for increased statistics from W9 (10mV noise threshold)
...
hTimeResCorr = all_histoInfos[0].th1
hTimet1 = all_histoInfos[1].th1
hTimet2 = all_histoInfos[2].th1
hTimeResW2 = all_histoInfos[3].th1 #7
hTimeResCorr.SetLineColor(kBlack)
hTimet1.SetLineColor(7)
hTimet1.SetLineWidth(6)
hTimet2.SetLineColor(9)
hTimeResW2.SetLineColor(416+2) #kGreen+2 #(TColor.GetColor(136,34,85))


ymin = hTimeResCorr.GetMinimum()
ymax = ylength

gPad.RedrawAxis("g")

hTimeResCorr.Draw("AXIS same")
hTimet1.Draw("hist e same")#
hTimet2.Draw("hist e same")#
...
legend.AddEntry(hTimet1, "t1 (w/ TrackerCorrection)")#
legend.AddEntry(hTimet2, "t2 (w/ TrackerCorrection)")#
