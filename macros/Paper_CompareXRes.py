# DS - As fas as I can see (21June25), code added to produce position resolution plots for the paper (midgap position at origin)
# DS - Scaling the (laser) position resolution curves to account for noise from both setups.
...
class HistoInfo:
    def __init__(self, inHistoName, f, outHistoName, doFits=True, yMax=30.0, title="", xlabel="", ylabel="Position resolution [#mum]", sensor="", shift=0.0, addShift = True):
        ...
        self.th1 = self.getTH1(self.th2, outHistoName, shift)

    def getTH2(self, f, name, sensor):
        th2 = f.Get(name)
        return th2

    def getTH1(self, th2, name, shift):
        th1_temp = TH1D(name,"",th2.GetXaxis().GetNbins(),th2.GetXaxis().GetXmin()-shift,th2.GetXaxis().GetXmax()-shift) # -centerShift
        return th1_temp
...
all_histoInfos = [
    ...
    HistoInfo("deltaX_vs_Xtrack_twoStrips",   inputfile, "track_twoStrips", True,  ylength, "", "Track x position [mm]","Position resolution [#mum]",dataset, shift=global_shift),
    # HistoInfo("deltaX_vs_Xtrack",   inputfile, "rms_track", False,  ylength, "", "Track x position [mm]","Position resolution RMS [#mum]",sensor),
]

if useTight:
    all_histoInfos = [
        HistoInfo("deltaX_vs_Xtrack_twoStrips_tight",   inputfile, "track_twoStrips%s"%pref_hotspot, True,  ylength, "", "Track x position [mm]","Position resolution [#mum]",dataset,shift=global_shift),
    ]
...
if debugMode:
    outdir_q = myStyle.GetPlotsDir(outdir, "q_Compare_Pres/")
...
for i in range(hist_info_twoStrip.th2.GetXaxis().FindBin(-0.5), hist_info_twoStrip.th2.GetXaxis().FindBin(0)):
    for info in all_histoInfos:
        ...
        if(nEvents > minEvtsCut):
            if(info.doFits):
                ...
                value = 1000.0*np.sqrt(mySigma*mySigma + myMPV*myMPV)
                # error = 1000.0*mySigmaError
                error = 1000 * TMath.Sqrt((myMPV*myMPV*myMPVError*myMPVError + mySigma*mySigma*mySigmaError*mySigmaError)/(myMPV*myMPV + mySigma*mySigma))
...
scaled_hist_info_twoStrip = hist_info_twoStrip.th1.Clone("scaled_h_twoStrip")

for number in range(1, hist_info_twoStrip.th1.GetXaxis().GetNbins()+1):
    pr = hist_info_twoStrip.th1.GetBinContent(number)
    pr_error = hist_info_twoStrip.th1.GetBinError(number)
    pr_scaled = pr*scaling_factor
    if "HPK_W5_17_2" not in dataset:
        pr_scaled_error = math.sqrt( (noise_ftbf*pr_error/noise_laser)**2 + (pr*noise_ftbf_error/noise_laser)**2 + (pr*noise_ftbf*noise_laser_error/(noise_laser)**2)**2 )
    else:
        pr_scaled_error = pr_error
    scaled_hist_info_twoStrip.SetBinContent(number, pr_scaled)
    scaled_hist_info_twoStrip.SetBinError(number, pr_scaled_error)

scaled_hist_info_twoStrip.Write()
outputfile.Close()
